...
- New: Engine: get variables or rule blocks by name using square brackets, eg, `engine["light"].value`
- New: Variable: get terms by name using square brackets, eg, `variable["low"].membership(value)`
- New: Factory: get constructors and objects by name using square brackets, eg, `factory["Triangle"]()`, `factory["sin"](3.14)`
- New: Qodana Static Code Analysis works locally, but not properly in Github, so the workflow is not enabled
- New: Benchmark class to benchmark engines
...
## benchmark.py
- New: Benchmark class to measure performance time and errors of engines

## defuzzifier.py
- Bug fix: Weighted defuzzifiers infer the type correctly now (see WeightedDefuzzifier::infer_type)
- Changed: Defuzzifier is abstract, and so are IntegralDefuzzifier and WeightedDefuzzifier
...
- New: `__getitem__` implemented to get variables or ruleblocks using square brackets
- New: Engine.infer_type to infer type of engine based on its configuration
- New: Engine.ready to check whether engine is ready to operate
- New: Engine.copy to deep copy an engine
...
- New: `__getitem__` implemented in `[Construction|Cloning]Factory` to get constructors and objects using square brackets
- New: `__len__` implemented in `[Construction|Cloning]Factory` to get number of constructors and objects using `len`
...
- New: Op.glob_examples method to glob over the fuzzylite examples
...
## rule.py
- New: `RuleBlock.__getitem__` gets the rule by index using square brackets `rb[0]`
- New: `RuleBlock.__len__` gets the number of rules the block has using `len(rb)`
...
- New: `__getitem__` implemented to get terms using square brackets, eg, `variable[0]` or `variable["low"]`
